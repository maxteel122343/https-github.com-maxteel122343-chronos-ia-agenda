...
import { CardData, Attachment, CardVisualSettings, CardBehaviorSettings, CardShape, CardColor, PomodoroConfig, PomodoroLap, CardPaneElement } from '../types';
...
const CardNode: React.FC<CardNodeProps> = React.memo(({
    ...
    const resetIdle = () => setLastInteraction(Date.now());
            ...
            setChronoCurrent(prev => {
                if (chronoDirection === 'up') {
                    ...
                    if (prev <= 0) {
                        ...
                        setChronoRunning(false);
                        setChronoFinished(true);
                        // Auto-record failure if time runs out on countdown
    ...
    if (card.type === 'note') {
        ...
        return (
            <div
                ...
                ${isSelected ? 'ring-2 ring-yellow-400 z-40 scale-105 shadow-xl' : 'z-10 hover:z-30 h-auto'}
            `}
                style={{
                    transform: `translate(${card.x}px, ${card.y}px) rotate(${isSelected ? 0 : rotation}deg)`,
            ...
            >
                ...
                {isSlideActive && card.slideSettings?.isEnabled && (
                    <SlideModeOverlay
                        ...
                        effect={card.slideSettings.transitionEffect}
                    />
                )}
                ...
                {(chronoMode ? showCornerChrono : chronoTarget > 0) && chronoTarget > 0 && (
                    <div className="absolute top-2.5 left-2.5 z-30 flex items-center gap-1 pointer-events-none" onClick={e => e.stopPropagation()}>
                ...
                <div className={`p-4 flex-1 flex flex-col overflow-y-auto custom-scrollbar relative transition-spacing duration-300 ${(chronoMode ? showCornerChrono : chronoTarget > 0) && !isEditing ? 'pt-10' : ''}`}>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                            {isEditing && (card.activePaneIndex || 0) === 0 ? (
                                ...
                                <div className="p-1 flex flex-col bg-white animate-in fade-in duration-300">
                                    ...
                                    <textarea
                                        className="bg-transparent resize-none focus:outline-none w-full placeholder-gray-300/80 text-gray-400 mt-1 min-h-[120px] leading-relaxed"
                                        style={{ fontSize: `${(settings.fontSize || 14) * 1.1}px` }}
                                        value={card.description}
                                        onChange={(e) => onUpdate(card.id, { description: e.target.value })}
                                        placeholder={chronoMode ? 'Digite 050 (progressivo) ou 50 (regressivo)...' : 'Write something...'}
                                        autoFocus
                                    />
                                        ...
                                        elementContent = (
                                            ...
                                            >
                                                ...
                                                {editingNoteId === elementId ? (
                                                    ...
                                                    <p
                                                        className="whitespace-pre-wrap leading-relaxed text-gray-400/80 cursor-text"
                                                        style={{ fontSize: `${(settings.fontSize || 14) * 1.05}px` }}
                                                        onClick={(e) => { e.stopPropagation(); setEditingNoteId(elementId); }}
                                                    >
                                        ...
                                        {!isEditing && (card.activePaneIndex || 0) === 0 && (
                                            <div className="flex flex-col mb-4">
                                                {!showExternalTitle && (card.title || isEditing) ? (
                                                    <h3 className="text-xs font-black uppercase tracking-widest text-gray-300/60 leading-tight">
                                                        {card.title || 'Note'}
                                                    </h3>
                                                ...
                                                {card.description ? (
                                                    <p className="whitespace-pre-wrap leading-relaxed text-gray-400/80" style={{ fontSize: `${(settings.fontSize || 14) * 1.1}px` }}>
                                                        {card.description}
                                                    </p>
                                                ) : (
                        ...
                        <>
                            {isEditing ? (
                                ...
                                <>
                                    ...
                                    <textarea
                                        className="bg-transparent flex-1 resize-none focus:outline-none w-full placeholder-gray-300 text-gray-600 mt-1 min-h-[100px]"
                                        style={{ fontSize: `${(settings.fontSize || 14) * 0.9}px` }}
                                        value={card.description}
                                        onChange={(e) => onUpdate(card.id, { description: e.target.value })}
                                        placeholder={chronoMode ? 'Digite 050 (progressivo) ou 50 (regressivo)...' : 'Write something...'}
                                        autoFocus
                                ...
                                <>
                                    ...
                                    {card.description && <p className="whitespace-pre-wrap leading-relaxed text-gray-500" style={{ fontSize: `${(settings.fontSize || 14) * 0.9}px` }}>{card.description}</p>}
                                </>
                            )}
                        </>
                    )}
                    ...
                    !isEditing && (
                        <div className="absolute -bottom-28 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 z-[60] animate-in slide-in-from-top-2 duration-300">
                            ...
                            <div className="flex items-center gap-3 bg-white/95 backdrop-blur-md border border-gray-100 px-4 py-2 rounded-full shadow-2xl shadow-black/10 transition-shadow hover:shadow-black/20">
                                ...
                                <div className="flex items-center gap-1.5 group/btn">
                                    <button
                                        onClick={(e) => { e.stopPropagation(); onCompleteTask?.(card.id); }}
                                ...
                                <div className="flex items-center gap-1.5 group/btn">
                                    <div className="w-7 h-7 rounded-full bg-amber-50 text-amber-800 flex items-center justify-center border border-amber-200" title="Incompleto">
    ...
    if (card.type === 'media' && card.attachments?.[0]) {
        ...
        return (
            <div
                ref={cardRef}
                className={`absolute group cursor-pointer ${isDragging ? '' : 'transition-all duration-200'} ${isSelected ? 'ring-4 ring-blue-500 z-40 scale-105' : 'z-10 hover:z-30 hover:ring-2 hover:ring-blue-300'}`}
                ...
                {isEditing ? (
                    ...
                    <>
                        ...
                        {settings.showDescription && card.description && (
                            <p
                                className="text-gray-400 line-clamp-2 min-h-[2rem]"
                                style={{
                                    fontFamily: settings.fontFamily || 'inherit',
                                    fontSize: `${(settings.fontSize || 14) * 0.9} px`
                                }}
                            >
                                {card.description}
                            </p>
...
const SlideModeOverlay: React.FC<{
    ...
    mediaType: string,
    effect: string
}> = ({ attachments, slideIndex, mediaType, effect }) => {
