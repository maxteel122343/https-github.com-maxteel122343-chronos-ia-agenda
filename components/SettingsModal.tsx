...
const SettingsModal: React.FC<SettingsModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-[5000] flex items-center justify-center p-4">
            ...
            {/* Modal Content */}
            <div className="relative bg-white/95 backdrop-blur-2xl border border-white/60 w-full max-w-4xl max-h-[85vh] rounded-[48px] shadow-[0_40px_100px_rgba(0,0,0,0.2)] flex flex-col overflow-hidden animate-in zoom-in-95 duration-500 cubic-bezier(0.19, 1, 0.22, 1)">
                ...
                <div className="px-12 py-10 border-b border-gray-100 flex justify-between items-center">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h2 className="text-4xl font-black text-gray-900 tracking-tighter leading-none">Settings</h2>
                            <p className="text-[11px] text-gray-400 font-black uppercase tracking-[0.2em] mt-3 ml-1 flex items-center gap-2">
                                <Sparkles size={12} className="text-blue-500" /> Advanced Control Center
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 flex overflow-hidden">
                    {/* Sidebar Nav */}
                    <div className="w-64 border-r border-gray-100 p-8 space-y-2">
                        {navItems.map(item => (
                            <button
                                key={item.id}
                                onClick={() => setActiveTab(item.id)}
                                className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl transition-all duration-300 font-bold text-sm ${activeTab === item.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-gray-400 hover:text-gray-900 hover:bg-gray-50'}`}
                            >
                                {item.icon}
                                {item.label}
                            </button>
                        ...
                        <div className="mt-12 pt-8 border-t border-gray-100 px-2">
                            <div className="flex items-center gap-3 text-[10px] font-black text-gray-300 uppercase tracking-widest bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                <Smartphone size={16} /> Device: Master
                            </div>
                        </div>
                    </div>
                ...
                {/* Footer Actions */}
                <div className="p-10 bg-gray-50/50 border-t border-gray-100 flex justify-between items-center px-12">
                    <div className="flex items-center gap-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                        <ShieldCheck size={16} className="text-emerald-500" /> All data is encrypted locally
                    </div>
                    <button
                        onClick={onClose}
                        className="bg-dark-900 text-white px-10 py-5 rounded-[24px] font-black text-sm uppercase tracking-widest shadow-2xl shadow-black/20 hover:scale-105 active:scale-95 transition-all"
                    >
